...
async def add_ie(user_id: int, tg_first_name: str, tg_last_name: str, name: str, email: str, password: str,
    ...
    try:
        ...
        logger.exception(f'Ошибка при добавлении ИП\n{e}')
...
async def get_bot_name(user_id: int):
    ...
    try:
        ...
        logger.exception(f'Ошибка при получении bot_name для пользователя {user_id}: {e}')
        return None  # or raise an exception if you prefer


async def update_bot_name(user_id: int, bot_name_new: str):
    try:
        user = await select_user(user_id)
        if user:
            await user.update(bot_name=bot_name_new).apply()
            return True  # Indicate successful update
        else:
            logger.warning(f"User with ID {user_id} not found. Cannot update bot_name.")
            return False  # Indicate that user was not found
    except Exception as e:
        logger.exception(f'Error updating bot_name for user {user_id}: {e}')
        return False  # Indicate an error during the update
